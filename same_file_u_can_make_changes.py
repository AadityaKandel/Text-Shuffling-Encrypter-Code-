...
def start_the_backup(called_or_not):
	...
	def thread1():
		...
		if called_or_not == False:
			l1.config(text="Backing Up Step 1/4",fg="green")
		else:
			l1.config(text="Clearing Data Step 1/5",fg="red")
		...
		if called_or_not == False:
			l1.config(text="Backing Up Step 2/4",fg="green")
		else:
			l1.config(text="Clearing Data Step 2/5",fg="red")
		...
		for i in range(1,total_files+1):
			...
			if os.path.exists(f'{lines_read[i]}') == True:
				...
				area.insert(i-1,lines_read[i])
				area.itemconfig(i-1,fg="green")
			
			else:

				l2.config(text=status)
				total_files=total_files-1
				area.insert(i-1,f'Cannot Find {lines_read[i]}')
				area.itemconfig(i-1,fg="red")
			area.yview_moveto(1)
		...
		if called_or_not == False:
			l1.config(text="Backing Up Step 3/3",fg="green")
		else:
			l1.config(text="Clearing Data Step 3/5",fg="red")

		l2.config(text="Backing Up Files")
		time.sleep(1)
		...
		for i in range(0,total_files):
			...
			try:
				...
				f.write(my_backup)
				f.close()
				status1 = f"Backing Up Files: ({i+1}/{total_files})"
				area.insert(i,f'Filename: {files_to_backup[backup_index+1::]}')
				area.itemconfig(i,fg="green")
				my_backup=""
		...
		if called_or_not == True:
			...
			l1.config(text="Clearing Data Step 4/5",fg="red")
			l2.config(text="Deleting Files [Checking....]")
			...
			l1.config(text="Clearing Data Step 5/5",fg="red")
			...
			l2.config(text=f"DATA CLEARING SUCCESSFUL WITH {errorlevel} BACKUP ERROR")

		else:

			l2.config(text=f"BACKUP SUCCESSFUL WITH {errorlevel} ERROR")

	l1 = Label(backup_root,text="Clearing Data [Step 1/5]",font="arial 12 bold",fg="green")
	l1.pack()
	l2 = Label(backup_root,text="Step 1: Finding Files",font="arial 12 bold",fg="green")
	l2.pack()

	area = Listbox(backup_root,height=30,width=80)

	scrollbar = Scrollbar(backup_root)
	scrollbar.pack(side=RIGHT, fill=Y)
	scrollbar.config(command=area.yview)

	scrollbar1 = Scrollbar(backup_root, orient=HORIZONTAL)
...
def multi_encrypt():
	...
	if save == "":
		...
		save_location = save[0]
		final_save_location = save_location[0:save_location.rindex('/')]

		if open_it == "":
			tmsg.showerror('Error','Location Was Not Given')

		elif final_save_location == open_it:
			tmsg.showerror('Error','Encrypted And Decrypted Files Cannot Be On The Same Location')
			...
			def multi_function():
				...
				for i in range(0, len(save)):
					...
					if error_detect > error_detectt:
						area.insert(i,f'[{multi_another_name}] [ERROR]')
						error_detectt=error_detectt+1
					else:
						saves(True)
						f = open(f'{open_it}/{multi_another_name}','w')
						# l2.config(text=f'[{multi_another_name}] -- ({i}/{len(save)})')
						area.insert(i,f'Filename: [{multi_another_name}]')
						l1.config(text=f"Encrypting Files ({i+1}/{len(save)})")
						f.write(final_text)
						f.close()
					area.itemconfig(i,fg="red")
					area.yview_moveto(1)

				l1.config(text=f'Encryption Complete with {error_detect} errors')
				root.title('Text Shuffling Encrypter')
				# multi_root.destroy()

			l1 = Label(multi_root,text="Encrypting Files",font="arial 12 bold",fg="green")
			l1.pack()

			area = Listbox(multi_root,height=30,width=80)

			scrollbar = Scrollbar(multi_root)
			scrollbar.pack(side=RIGHT, fill=Y)
			scrollbar.config(command=area.yview)

			scrollbar1 = Scrollbar(multi_root, orient=HORIZONTAL)
			scrollbar1.pack(side=BOTTOM, fill=X)
			scrollbar1.config(command=area.xview)
...
Remember That It Only Works For The Files That You Have Already Encrypted')
	...
	if save == "":
		...
		if open_it == "":
			...
			tmsg.showerror('Error','Decryption Process Cannot Be Completed On The Same Location')
			...
			def multi_decrypt_function():
				...
				for i in range(0, len(save)):
					...
					if errorlevel > error_detect:
						area.insert(i,f"\nFilename: [ERROR] [{decrypted_items_name}]")
						area.itemconfig(i,fg="red")
						error_detect=error_detect+1
					else:
						area.insert(i,f"\nFilename: [{decrypted_items_name}]")
						area.itemconfig(i,fg="green")
					l1.config(text=f"Decrypting Files ({i+1}/{len(save)})")
					area.yview_moveto(1)

					f.write(decrypted_item)
					f.close()

				l1.config(text=f'Decryption Complete with {errorlevel} error')
				# multi_decrypt_root.destroy()

			l1 = Label(multi_decrypt_root,text="Decrypting Files",font="arial 12 bold",fg="green")
			l1.pack()

			area = Listbox(multi_decrypt_root,height=30,width=80)

			scrollbar = Scrollbar(multi_decrypt_root)
			scrollbar.pack(side=RIGHT, fill=Y)
			scrollbar.config(command=area.yview)

			scrollbar1 = Scrollbar(multi_decrypt_root, orient=HORIZONTAL)
			scrollbar1.pack(side=BOTTOM, fill=X)
			scrollbar1.config(command=area.xview)
